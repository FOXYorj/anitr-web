...
func PlayWithPlaylist(params []MPVParams, startIndex int) (*exec.Cmd, string, error) {
	...
	// Eğer başlangıç bölümünün bir pozisyonu varsa --start ekle
	if params[startIndex].StartPositionSec > 0 {
		args = append(args, fmt.Sprintf("--start=%.0f", params[startIndex].StartPositionSec))
	}
