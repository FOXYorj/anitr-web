...
func (a AnimeCix) GetSearchData(query string) ([]models.Anime, error) {
	...
	for _, item := range data {
		...
		// Açıklama için farklı keyleri dene
		synopsis, _ := item["description"].(string)
		if synopsis == "" {
			synopsis, _ = item["overview"].(string)
		}
		if synopsis == "" {
			synopsis, _ = item["synopsis"].(string)
		}
...
func (a AnimeCix) GetAnimeByID(idstr string) (*models.Anime, error) {
	...
	synopsis, _ := titleMap["description"].(string)
	if synopsis == "" {
		synopsis, _ = titleMap["overview"].(string)
	}
	if synopsis == "" {
		synopsis, _ = titleMap["synopsis"].(string)
	}
...
func (a AnimeCix) GetEpisodesData(params models.EpisodeParams) ([]models.Episode, error) {
	// Sezon indexi belirle (varsayılan 0)
	targetSeasonIdx := 0
	if params.SeasonNum != nil {
		targetSeasonIdx = *params.SeasonNum
	}

	// Sadece hedef sezonu çek
	seasons, err := FetchAnimeSeasonsData(*params.SeasonID)
	if err != nil {
		return nil, fmt.Errorf("sezon verileri alınamadı: %w", err)
	}

	// Geçerli sezon indeksi kontrolü
	if targetSeasonIdx >= len(seasons) {
		targetSeasonIdx = 0
	}
	seasonIdx := seasons[targetSeasonIdx]

	url := fmt.Sprintf("%ssecure/related-videos?episode=1&season=%d&titleId=%d&videoId=637113", configAnimecix.AlternativeUrl, seasonIdx+1, *params.SeasonID)
	data, err := internal.GetJson(url, configAnimecix.HttpHeaders)
	if err != nil {
	...
	if !ok {
		return nil, fmt.Errorf("'videos' verisi yok")
	}

	seen := make(map[string]bool)
	var episodes []models.Episode
	for i, video := range videosRaw {
		v, ok := video.(map[string]interface{})
		if !ok {
			continue
		}
		...
		seen[name] = true
		episodeUrl, _ := v["url"].(string)
		seasonNum := v["season_num"]
		episode := models.Episode{
			ID:     episodeUrl,
			Title:  name,
			Number: i + 1,
			Extra:  map[string]interface{}{"season_num": seasonNum},
		}
...
func (a AnimeCix) GetWatchData(req models.WatchParams) ([]models.Watch, error) {
	// Verilerin eksik olup olmadığını kontrol et (sadece zorunlu alanlar)
	if req.IsMovie == nil || req.Id == nil {
		return nil, fmt.Errorf("eksik izleme parametreleri (isMovie, id zorunlu)")
	}

	// Dizi ise url zorunlu
	if !*req.IsMovie && req.Url == nil {
		return nil, fmt.Errorf("eksik izleme parametreleri (dizi için url zorunlu)")
	}
	...
	// Extra parametrelerini güvenli şekilde al (panic'e yol açmaması için)
	seasonIndex := 0
	episodeIndex := 0
	if req.Extra != nil {
		Extra := *req.Extra
		if si, ok := Extra["seasonIndex"]; ok {
			if siv, ok2 := si.(int); ok2 {
				seasonIndex = siv
			}
		}
		if ei, ok := Extra["episodeIndex"]; ok {
			if eiv, ok2 := ei.(int); ok2 {
				episodeIndex = eiv
			}
		}
	}
...
func FetchTRCaption(seasonIndex, episodeIndex, id int) (string, error) {
	...
	if !ok || len(videosSlice) == 0 {
		return "", fmt.Errorf("'videos' verisi yok veya boş")
	}

	// Sınır kontrolü - geçersiz index olursa hata döndür
	if episodeIndex < 0 || episodeIndex >= len(videosSlice) {
		return "", fmt.Errorf("episodeIndex sınırların dışında: %d (toplam: %d)", episodeIndex, len(videosSlice))
	}
...
func AnimeMovieWatchApiUrl(id int) (map[string]interface{}, error) {
	...
	for _, video := range videosRaw {
		...
		if !ok {
			log.Println("'videos' verisi beklenen formatta değil")
			continue
		}
		...
		if !ok {
			log.Println("'url' verisi beklenen formatta değil")
			continue
		}
		...
		if err != nil {
			log.Printf("video verileri alınamadı: %v", err)
			continue
		}
		...
		parsedUrl, err := url.Parse(finalUrl)
		if err != nil {
			log.Printf("URL ayrıştırma hatası: %v", err)
			continue
		}
		...
		response.Body.Close()
		if err != nil {
			log.Printf("video verileri okunamadı: %v", err)
			continue
		}
		...
		if err != nil {
			log.Printf("video verileri ayrıştırılamadı: %v", err)
			continue
		}
		...
		captions, ok := video["captions"].([]interface{})
		if !ok {
			log.Println("'captions' verisi beklenen formatta değil")
			return result, nil
		}
		...
		for i, caption := range captions {
			captionMap, ok := caption.(map[string]interface{})
			if !ok {
				continue
			}

			urlStr, ok := captionMap["url"].(string)
			if !ok {
				continue
			}
			...
			lang, ok := captionMap["language"].(string)
			if ok && lang == "tr" {
				trCaption = urlStr
				break
			}
		...
		if trCaption != "" {
			result["caption_url"] = trCaption
		} else {
			result["caption_url"] = firstCaption
		}
