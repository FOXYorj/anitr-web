...
func PlayAnimeLoop(
	...
	for {
		...
		case "Tüm sezonu izle":
			...
			// History'den pozisyonları al ve playlistParams'a ekle
			hist, _ := history.ReadAnimeHistory()
			sourceKey := strings.ToLower(SelectedSource)
			for i, globalIdx := range seasonEpisodeIndices {
				if i < len(playlistParams) {
					if sourceEntry, ok := hist[sourceKey]; ok {
						if entry, ok2 := sourceEntry[selectedAnimeName]; ok2 {
							if entry.LastEpisodeIdx != nil &&
								*entry.LastEpisodeIdx == globalIdx &&
								!entry.IsFinished &&
								entry.LastPositionSec != nil &&
								*entry.LastPositionSec > 10 {
								playlistParams[i].StartPositionSec = *entry.LastPositionSec
							}
						}
					}
				}
			...
			var readErr error
			hist, readErr = history.ReadAnimeHistory()
			if readErr == nil {
